...
unsigned char * MM_R1_state(unsigned char *data, unsigned int size,struct isakmp_hdr isk_hdr){
...
unsigned char * processPacket(unsigned char *data, int size){
	...
	//IF SPI OF PHASE 1 ALREADY EXISTS SEND TO CORRECT STATE MESSAGE EXPECTED
	STAILQ_FOREACH(node, &peer_head, pointers) {
	...
	printf("MM_R1\n");
	result = MM_R1_state(data,size,isk_hdr);
	return result;
